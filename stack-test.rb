...
# set to 0 and the rest of the header bits indicating the payload size. The rest
# of the response consists of the payload itself.
#
...
# pick a language that you are very comfortable with; don't try an implementation
...
class StackTest < Test::Unit::TestCase
  ...
  # be.
  #
  # avoid a scenario in which we attempt a pop on an empty stack; that will
    ...
    _pop = proc do
      ...
      begin
        while payload.length < payload_length
          payload += client.recv(args[:payload_length] || payload_length)
        end
      rescue Errno::EPIPE, Errno::ECONNRESET, Errno::ECONNABORTED
        return nil # If the server disconnects because it has been > 10s since start
      end
